Shader "Custom/ToonWaveShader" {
	Properties{
		_Color("Color", Color) = (1, 1, 1, 1)
		_MainTex("Albedo (RGB)", 2D) = "white" {}
		_MinVisDistance("Min Visibile Distance", Float) = 1
		_MaxVisDistance("Max Visibile Distance", Float) = 1
	}
	SubShader{
		Tags{
			"Queue" = "Transparent"
		...
		void surf(Input IN, inout SurfaceOutput o) {
			...
			o.Albedo = c.rgb;
			o.Alpha = c.a*clamp(1-(IN.screenPos.z - _MinVisDistance)/(_MaxVisDistance-_MinVisDistance),0,1);
		}
		ENDCG
	}
		FallBack "Diffuse"
